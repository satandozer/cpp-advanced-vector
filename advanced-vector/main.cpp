...
struct Obj {
    ...
    explicit Obj(int id_)
        : id(id_)  //
    {
        ++num_constructed_with_id;
    }

    Obj(int id_, std::string name_)
        : id(id_)
        , name(std::move(name_))  //
    {
...
/*
void FillVector(Vector<Obj>& v){
    for (size_t i = 0; i != v.Size(); ++i){
        v[i].id = static_cast<int>(i);
    }  
}

void PrintVector(const Vector<Obj>& v){
    for (size_t i = 0; i != v.Size(); ++i){
        std::cerr << '[' << v[i].id << ':' << v[i].name << ']';
    }
    std::cerr << std::endl;
}
*/
...
int main() {
    try {
        Test1();
        std::cerr << "Test 1 OK" << std::endl;
        Test2();
        std::cerr << "Test 2 OK" << std::endl;
        Test3();
        std::cerr << "Test 3 OK" << std::endl;
        Test4();
        std::cerr << "Test 4 OK" << std::endl;
        Test5();
        std::cerr << "Test 5 OK" << std::endl;
        Test6();
        std::cerr << "Test 6 OK" << std::endl;
        Benchmark();
        std::cerr << "Benchmark test OK" << std::endl;
